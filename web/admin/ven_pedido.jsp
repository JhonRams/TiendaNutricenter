                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <table class="table table-striped">
                            ...
                            <tbody>
                                <%
                                    for (venta elem : _venta.Ver_x_estado("pedido")) {
                                %>
                                <tr>
                                    <th scope="row"><%=elem.getId()%></th>
                                    <td><%=elem.getNumero_pago()%></td>
                                    <td><%=elem.getTotal()%></td>
                                    <td><%=elem.getFecha_pedido()%></td>
                                    <td><%=elem.getObjCliente().getNombre()%></td>
                                    <td><%=elem.getObjDistrito().getNombre()%></td>
                                    <td><a href="?pedido=<%=elem.getId()%>" class="btn btn-warning">Atender</a></td>
                                </tr>
                                <%
                                    }
                                %>
...
<section class="full-tvn-1 " id="vtn-personal-nuevo">
    ...
    <div class="admin-editor">
        ...
        <div class="pad1">
            ...
            <div>Delivery:  <%=objventa.getObjdelivery().horario() %> / <%=objventa.getObjdelivery().getObjPersonal().getNombre() %></div>
        </div>
        <div class="card-body">
        ...
        <form method="POST" action="../usuario_ventas" class="text-right pad2">
            <input type="hidden" value="<%=objventa.getId()%>" name="idpedido">
            <button class="btn btn-success">Concretar Pedido</button>
        </form>
    </div>
</section>
